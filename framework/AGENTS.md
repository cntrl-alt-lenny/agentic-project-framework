...
other's work. {{HUMAN_HANDLE}} edits it indirectly by telling the
...
{{HUMAN_HANDLE}} opens a chat and says *"you are the decomper"*, that
...
one-time steps before you touch any PR. {{HUMAN_HANDLE}} will
...
comment, update `docs/state.md`, write briefs for {{HUMAN_HANDLE}} to
...
|------------------------------------------------|--------------|--------------------------------------------------------------------------|
| `~/Dev/{{PROJECT_DIR}}/brain`                  | `brain`      | Main repo (owns `.git/`). Brain pulls main, reviews PRs, builds verifications. |
| `~/Dev/{{PROJECT_DIR}}/decomper`               | `decomper`   | Sibling worktree. Decomper checks out its own `decomper/<scope>` branches without touching brain's working state. |
| `~/Dev/{{PROJECT_DIR}}/scaffolder`             | `scaffolder` | Sibling worktree. Scaffolder checks out its own `scaffolder/<scope>` branches the same way. |

Add the sibling worktrees once per machine:

```bash
git worktree add ~/Dev/{{PROJECT_DIR}}/decomper   main
git worktree add ~/Dev/{{PROJECT_DIR}}/scaffolder main
...
```
~/Dev/{{PROJECT_DIR}}/brain/   (or wherever the brain checkout lives)
...
2. **Never push to `main` directly.** Every change is a pull request.
   The brain reviews locally, summarizes to {{HUMAN_HANDLE}} in plain
   English, and merges on OK. {{HUMAN_HANDLE}} retains veto — the
...
4. **Stay inside your "Owns" column.** If the task needs a change in
   ...
   (as them, not you) or ask {{HUMAN_HANDLE}} / the brain to
   re-partition.
5. **Open a PR when done.** Don't merge your own PR — that's the
   brain's job (including for brain-authored PRs, on
   {{HUMAN_HANDLE}}'s OK). Don't force-push. Describe in the PR body:
...
4. **Brain reviews locally** — pulls the branch, runs the project's
   verification subset, then summarizes for {{HUMAN_HANDLE}} in plain
   English: what changed, why it's safe, what's next.
   {{HUMAN_HANDLE}} doesn't need to read the diff — the summary is
   the interface.
5. **Brain offers to merge.** On {{HUMAN_HANDLE}}'s OK (explicit
   "merge it" or a thumbs-up), merge with `gh pr merge <N> --squash
   --delete-branch`. When {{HUMAN_HANDLE}} is AFK, the brain
...
{{HUMAN_HANDLE}} says, in plain English, something like:
...
2. Moves any overlapping paths off other agents so nothing is
   double-owned.
3. Opens a PR with the change. {{HUMAN_HANDLE}} merges.
